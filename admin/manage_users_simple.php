...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($row['id'] != $_SESSION['user_id']) {
        // Add creation type information
        $row['creation_source'] = ($row['role'] === 'buyer') ? 'Coupon Assignment' : 'Manual Creation';
        $users[] = $row;
    }
}
                            ...
                            <tbody>
                                <?php foreach ($users as $u): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($u['role'] === 'buyer'): ?>
                                                <span class="badge bg-info">Coupon Assignment</span>
                                            <?php else: ?>
                                                <span class="badge bg-secondary">Manual Creation</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <?php if ($u['role'] !== 'admin'): ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" action="" id="create-user-form">
                    ...
                    <!-- Fields only shown for buyers and recipients -->
                    <div class="buyer-recipient-fields" style="display:none;">
                        <div class="mb-3">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show/hide buyer/recipient fields based on role selection
        const roleSelect = document.getElementById('role');
        roleSelect.addEventListener('change', function() {
            const buyerRecipientFields = document.querySelector('.buyer-recipient-fields');
            if (this.value === 'buyer' || this.value === 'recipient') {
                buyerRecipientFields.style.display = 'block';
            } else {
                buyerRecipientFields.style.display = 'none';
            }
        });
    });
</script>
