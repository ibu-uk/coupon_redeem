...
if (isset($_POST['lookup_coupon']) && validateFormToken('lookup_coupon', $_POST['lookup_token'])) {
    ...
    if (empty($couponCode)) {
        ...
        // Handle special case for numeric input with coupon type selection
        ...
        // Special case for B101
        else if ($couponCode == "B101" || (is_numeric($couponCode) && (empty($selectedType) || $selectedType == 'black'))) {
            $query = "SELECT c.*, ct.name as coupon_type_name, ct.value as coupon_type_value,
                     b.full_name as buyer_name, b.civil_id as buyer_civil_id, 
                     b.mobile_number as buyer_mobile, b.file_number as buyer_file_number
              FROM coupons c
            ...
            $debug .= "Using special case for B{$couponCode}<br>";
        }
        else {
            // Full code provided (e.g. B101, G105, etc.)
            $query = "SELECT c.*, ct.name as coupon_type_name, ct.value as coupon_type_value,
                     b.full_name as buyer_name, b.civil_id as buyer_civil_id, 
                     b.mobile_number as buyer_mobile, b.file_number as buyer_file_number
              FROM coupons c
        ...
        if ($result['count'] > 0) {
            ...
            if (empty($serviceName) || empty($serviceDescription) || empty($amount) || 
                ...
                if ($stmt->execute()) {
                    ...
                    if ($stmt->execute()) {
                        $debug .= "Coupon balance updated successfully<br>";
                        $success = "Coupon redeemed successfully!";
                        // Reset coupon data
                        $couponData = null;
                    } else {
                        $debug .= "Failed to update coupon balance<br>";
                        $error = "Failed to update coupon balance.";
                    }
                } else {
                    $debug .= "Failed to create redemption log<br>";
...
<head>
    ...
    <title>Simple Coupon Redemption</title>
...
<body>
    ...
    <div class="container mt-4">
        <h1 class="mb-4">Simple Coupon Redemption</h1>
            ...
            <div class="card-body">
                <form method="post" action="">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="coupon_code" name="coupon_code" placeholder="Enter coupon number (e.g. 101) or full code (e.g. B101)" required>
                    ...
                    <div class="mb-3">
                        ...
                        <small class="text-muted">This helps disambiguate when you enter just a number (e.g., "1" could be B101, G101, or S101)</small>
                    </div>
